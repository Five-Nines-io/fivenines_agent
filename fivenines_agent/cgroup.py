...
def _v1_cpu_controller():
    ...
    for candidate in ("cpuacct", "cpu,cpuacct"):
        ...
        # Combined v1 mounts are arbitrary comma-joined controller sets
        # (cpuacct,cpu or larger); scan for any mount whose components
        # include cpuacct rather than hardcoding the two common names.
        try:
            entries = sorted(os.listdir(CGROUP_ROOT))
        except OSError:
            entries = []
...
def detect_hierarchy_uncached():
    """One detection pass, no caching.

    Single source of the v2/v1/None decision: the cached detect_hierarchy()
    below (collector view) and the permission probe (capability view) both
...
def detect_hierarchy():
    ...
    _cached_hierarchy = detect_hierarchy_uncached()
