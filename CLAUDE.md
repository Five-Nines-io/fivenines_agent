...
- Agent reads `TOKEN` file from config directory (`/etc/fivenines_agent` for system install, `~/.local/fivenines/config` for user install; overridable via `CONFIG_DIR` env var)
...
### Config Validation (`config_schema.py`)
All API config is validated and sanitized by `validate_config()` before use. When adding a new config key or collector:
- **Boolean feature flags**: add the key to the `for key in (...)` loop in `validate_config()`
- **Collectors with `pass_kwargs=True`**: add a `_sanitize_<name>()` function and register it in `collector_sanitizers` inside `validate_config()`. Enforce loopback-only for any host/URL params, strip CRLF from passwords, and validate port ranges.
- **Other keys**: handle explicitly in `validate_config()` — unknown keys are silently dropped.
- Always add corresponding tests in `tests/test_config_schema.py`.
