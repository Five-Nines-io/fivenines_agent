...
- Patterns are globs; a name with no unit suffix gets systemd's implicit `.service` on BOTH sides of the match, so `nginx` and `nginx.service` are one entry. A glob, a PATH or a leading dash in a name the SERVER sent is REFUSED, never matched literally: `journalctl -u` expands globs itself (policy `app-?.service` would otherwise accept a requested `app-*.service` and read every app unit), it resolves `/var/log` to `var-log.mount` (so the policy would be checking a different string than journalctl reads), and a leading `-` would be parsed as an option rather than as the argument to `-u`. None of the three is legal in a systemd unit name. Also refused: a dangling symlink (a broken policy is not an absent one), a non-regular file (open() on a FIFO blocks the watchdog-bounded loop forever), a file over `MAX_FILE_BYTES` (truncating a blob can leave a partial `*.service` -> `*`, which WIDENS), and a unit name over `MAX_UNIT_CHARS`. `filter_units` returns (allowed, refused) and `logs` ships `refused_units` in the payload, because `units: {}` is byte-identical to "every unit was healthy". The server's list is itself bounded per call by `MAX_FILTER_UNITS` (200, the `MAX_PING_TARGETS`/`MAX_RESCAN_SCANNED` posture: each entry costs up to `MAX_PATTERNS` fnmatch calls on the watchdog-bounded loop), and everything past the cap joins `refused` rather than vanishing -- a server list long enough to hit it would otherwise drop the operator's genuinely-allowed units with no line anywhere
