...
def test_banner_cgroup_in_services_section():
    """cgroup gates the systemd collector's per-unit metrics, so it sits in
    the Services group next to systemd (kernel surface, not a sensor)."""
    output = _print_banner_with_capabilities(_full_caps(cgroup="v2"))
    services_idx = output.index("Services:")
    cgroup_idx = output.index("Cgroup")
    assert services_idx < cgroup_idx
    # And it moved OUT of Hardware Sensors: nothing cgroup-ish between the
    # Hardware header and the next section header (Storage).
    hardware_start = output.index("Hardware Sensors:")
    storage_start = output.index("Storage:")
    assert "Cgroup" not in output[hardware_start:storage_start]
