...
# --- cgroup_dir (ControlGroup-based paths) ---


def test_cgroup_dir_v2_default_slice():
    assert cgroup_dir("/system.slice/nginx.service", "v2") == (
        "/sys/fs/cgroup/system.slice/nginx.service"
    )


def test_cgroup_dir_v2_non_default_slice():
    """The fix: a unit relocated with Slice= resolves to its real path, not
    a hard-coded system.slice."""
    assert cgroup_dir("/machine.slice/foo.service", "v2") == (
        "/sys/fs/cgroup/machine.slice/foo.service"
    )


def test_cgroup_dir_v2_nested_slice():
    assert cgroup_dir("/system.slice/db.slice/pg.service", "v2") == (
        "/sys/fs/cgroup/system.slice/db.slice/pg.service"
    )


def test_cgroup_dir_v1_requires_controller():
    with pytest.raises(ValueError, match="controller is required"):
        cgroup_dir("/system.slice/nginx.service", "v1")


def test_cgroup_dir_v1_with_controller():
    assert cgroup_dir("/system.slice/nginx.service", "v1", controller="memory") == (
        "/sys/fs/cgroup/memory/system.slice/nginx.service"
    )


def test_cgroup_dir_v1_non_default_slice():
    assert cgroup_dir("/machine.slice/foo.service", "v1", controller="memory") == (
        "/sys/fs/cgroup/memory/machine.slice/foo.service"
    )


def test_cgroup_dir_unknown_hierarchy():
    with pytest.raises(ValueError, match="unsupported hierarchy"):
        cgroup_dir("/system.slice/nginx.service", "v3")


def test_cgroup_dir_rejects_empty():
    with pytest.raises(ValueError, match="invalid control group"):
        cgroup_dir("", "v2")


def test_cgroup_dir_rejects_relative():
    """ControlGroup must be absolute; a relative value cannot escape the root."""
    with pytest.raises(ValueError, match="invalid control group"):
        cgroup_dir("system.slice/nginx.service", "v2")


def test_cgroup_dir_rejects_path_traversal():
    with pytest.raises(ValueError, match="invalid control group"):
        cgroup_dir("/system.slice/../../etc/passwd", "v2")


def test_cgroup_dir_rejects_null_byte():
    with pytest.raises(ValueError, match="invalid control group"):
        cgroup_dir("/system.slice/nginx\x00.service", "v2")


def test_cgroup_dir_accepts_systemd_special_chars():
    """systemd unit names with @ and : characters are valid."""
    p = cgroup_dir("/system.slice/getty@tty1.service", "v2")
...
def test_read_memory_current_v2_happy():
    with patch("builtins.open", mock_open(read_data="123456789\n")):
        assert read_memory_current("/system.slice/nginx.service", "v2") == 123456789


def test_read_memory_current_v1_happy():
    with patch("builtins.open", mock_open(read_data="987654321\n")):
        assert read_memory_current("/system.slice/nginx.service", "v1") == 987654321
...
def test_read_memory_current_max_returns_none():
    ...
    with patch("builtins.open", mock_open(read_data="max\n")):
        assert read_memory_current("/system.slice/nginx.service", "v2") is None
...
def test_read_memory_current_permission_denied_silent():
    ...
    with patch("builtins.open", side_effect=PermissionError):
        with patch("fivenines_agent.cgroup.log") as mock_log:
            result = read_memory_current("/system.slice/nginx.service", "v2")
...
def test_read_memory_current_unsupported_hierarchy():
    assert read_memory_current("/system.slice/nginx.service", None) is None
...
def test_read_cpu_usec_v2_happy():
    with patch("builtins.open", mock_open(read_data=CPU_STAT_V2)):
        assert read_cpu_usec("/system.slice/nginx.service", "v2") == 5000000


def test_read_cpu_usec_v2_missing_key():
    with patch("builtins.open", mock_open(read_data="user_usec 100\n")):
        assert read_cpu_usec("/system.slice/nginx.service", "v2") is None
...
def test_read_cpu_usec_v1_converts_ns_to_usec():
    ...
    with patch("fivenines_agent.cgroup._v1_cpu_controller", return_value="cpuacct"):
        with patch("builtins.open", mock_open(read_data="5000000000\n")):
            ...
            assert read_cpu_usec("/system.slice/nginx.service", "v1") == 5000000
...
def test_read_cpu_usec_v1_combined_mount():
    ...
    with patch("fivenines_agent.cgroup._v1_cpu_controller", return_value="cpu,cpuacct"):
        with patch("builtins.open", side_effect=fake_open):
            assert read_cpu_usec("/system.slice/nginx.service", "v1") == 3000000
...
def test_read_cpu_usec_v1_no_cpu_controller():
    ...
    with patch("fivenines_agent.cgroup._v1_cpu_controller", return_value=None):
        with patch("builtins.open") as mock_o:
            assert read_cpu_usec("/system.slice/nginx.service", "v1") is None
...
def test_read_cpu_usec_unsupported_hierarchy():
    assert read_cpu_usec("/system.slice/nginx.service", None) is None
...
def test_v1_cpu_controller_neither_mount_returns_none():
    with patch("fivenines_agent.cgroup.os.path.isdir", return_value=False):
        with patch("fivenines_agent.cgroup.os.listdir", return_value=["memory"]):
            assert _v1_cpu_controller() is None


def test_v1_cpu_controller_scans_exotic_combined_mounts():
    """Arbitrary comma-joined controller sets (cpuacct,cpu or larger) are
    found by scanning the cgroup root, not just the two common names."""

    def fake_isdir(p):
        return p.endswith("/net_cls,cpuacct,cpu")

    with patch("fivenines_agent.cgroup.os.path.isdir", side_effect=fake_isdir):
        with patch(
            "fivenines_agent.cgroup.os.listdir",
            return_value=["memory", "net_cls,cpuacct,cpu"],
        ):
            assert _v1_cpu_controller() == "net_cls,cpuacct,cpu"


def test_v1_cpu_controller_scan_oserror_returns_none():
    with patch("fivenines_agent.cgroup.os.path.isdir", return_value=False):
        with patch("fivenines_agent.cgroup.os.listdir", side_effect=OSError):
            assert _v1_cpu_controller() is None
...
def test_read_oom_kill_count_v2_happy():
    with patch("builtins.open", mock_open(read_data=MEMORY_EVENTS_V2)):
        assert read_oom_kill_count("/system.slice/nginx.service", "v2") == 3


def test_read_oom_kill_count_v1_returns_none():
    """v1 has no memory.events equivalent; collector ships null."""
    assert read_oom_kill_count("/system.slice/nginx.service", "v1") is None


def test_read_oom_kill_count_none_hierarchy():
    assert read_oom_kill_count("/system.slice/nginx.service", None) is None


def test_read_oom_kill_count_missing_key():
    with patch("builtins.open", mock_open(read_data="low 0\nhigh 0\n")):
        assert read_oom_kill_count("/system.slice/nginx.service", "v2") is None
...
def test_read_inception_id_v2_happy():
    ...
    with patch("fivenines_agent.cgroup.os.stat", return_value=fake_stat) as mock_stat:
        result = read_inception_id("/system.slice/nginx.service", "v2")
    assert result == 42
...
def test_read_inception_id_v1_uses_memory_controller():
    ...
    with patch("fivenines_agent.cgroup.os.stat", return_value=fake_stat) as mock_stat:
        result = read_inception_id("/system.slice/nginx.service", "v1")
    assert result == 99
...
def test_read_inception_id_no_hierarchy():
    assert read_inception_id("/system.slice/nginx.service", None) is None


def test_read_inception_id_file_not_found():
    with patch("fivenines_agent.cgroup.os.stat", side_effect=FileNotFoundError):
        assert read_inception_id("/system.slice/nginx.service", "v2") is None
...
def test_read_unit_resources_v2_aggregates_all_metrics():
    ...
    with patch("builtins.open", side_effect=fake_open):
        with patch("fivenines_agent.cgroup.os.stat", return_value=fake_stat):
            result = read_unit_resources("/system.slice/nginx.service", "v2")
    assert result == {
        "memory_current": 1024,
        "cpu_usec": 5000000,
        "oom_kill_count": 3,
...
def test_read_unit_resources_v1_no_oom():
    ...
    with patch("fivenines_agent.cgroup._v1_cpu_controller", return_value="cpuacct"):
        with patch("builtins.open", side_effect=fake_open):
            with patch("fivenines_agent.cgroup.os.stat", return_value=fake_stat):
                result = read_unit_resources("/system.slice/nginx.service", "v1")
    assert result == {
        "memory_current": 2048,
        "cpu_usec": 3000000,
        "oom_kill_count": None,
...
def test_read_unit_resources_no_hierarchy_returns_empty():
    assert read_unit_resources("/system.slice/nginx.service", None) == {}


def test_read_unit_resources_unsupported_hierarchy_returns_empty():
    assert read_unit_resources("/system.slice/nginx.service", "v9") == {}


def test_read_unit_resources_empty_control_group_returns_empty():
    """An inactive/transient unit has no live cgroup; no reads attempted."""
    with patch("builtins.open") as mock_o:
        with patch("fivenines_agent.cgroup.os.stat") as mock_stat:
            assert read_unit_resources("", "v2") == {}
    mock_o.assert_not_called()
    mock_stat.assert_not_called()


def test_read_unit_resources_non_default_slice_reads_real_path():
    """The fix end-to-end: metrics for a Slice=-relocated unit come from its
    actual cgroup path, not a system.slice guess."""
    fake_stat = MagicMock(st_ino=11)
    captured = []

    def fake_open(path, *_args, **_kwargs):
        captured.append(path)
        if path.endswith("memory.current"):
            return mock_open(read_data="4096\n").return_value
        if path.endswith("cpu.stat"):
            return mock_open(read_data=CPU_STAT_V2).return_value
        if path.endswith("memory.events"):
            return mock_open(read_data=MEMORY_EVENTS_V2).return_value
    ...
    with patch("builtins.open", side_effect=fake_open):
        with patch("fivenines_agent.cgroup.os.stat", return_value=fake_stat):
            result = read_unit_resources("/machine.slice/foo.service", "v2")
    assert result["memory_current"] == 4096
    assert result["inception_id"] == 11
    # Every read targeted the machine.slice path, not system.slice.
    assert all("/machine.slice/foo.service" in p for p in captured)
    assert not any("system.slice" in p for p in captured)
