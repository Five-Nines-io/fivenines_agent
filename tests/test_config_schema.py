...
def test_clamp_in_range():
    assert _clamp(100, INTERVAL_MIN, INTERVAL_MAX, 60) == 100


def test_clamp_below_min():
    assert _clamp(10, INTERVAL_MIN, INTERVAL_MAX, 60) == INTERVAL_MIN
...
def test_sanitize_proxmox_forces_verify_ssl_true():
    result = _sanitize_proxmox({"host": "localhost", "verify_ssl": False})
    assert result is not None
    assert result["verify_ssl"] is True


def test_sanitize_proxmox_logs_warning_for_verify_ssl_false():
    with patch("fivenines_agent.config_schema.log") as mock_log:
        _sanitize_proxmox({"host": "localhost", "verify_ssl": False})
        mock_log.assert_any_call(
            "config_schema: proxmox verify_ssl=False rejected, forcing True", "warn"
        )


def test_sanitize_proxmox_verify_ssl_true_no_warning():
    with patch("fivenines_agent.config_schema.log") as mock_log:
        result = _sanitize_proxmox({"host": "localhost", "verify_ssl": True})
        assert result is not None
        assert result["verify_ssl"] is True
        for call in mock_log.call_args_list:
            assert "verify_ssl" not in str(call)
...
def test_validate_config_interval_clamped_low():
    result = validate_config({"interval": 5})
...
def test_validate_config_interval_in_range():
    result = validate_config({"interval": 120})
